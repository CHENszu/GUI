...
def select_game_region(status_callback=None, save_to_file=True, msg_prefix="截图区域"):
    clicks = []
    ...
    def on_click(x, y, button, pressed):
        if pressed and button == mouse.Button.left:
            ...
            if len(clicks) == 1:
                msg = f"[{msg_prefix}] 已记录左上角: ({int(x)}, {int(y)})。请点击右下角..."
                if status_callback:
                    status_callback(msg)
                else:
                    print(msg)
            elif len(clicks) == 2:
                msg = f"[{msg_prefix}] 已记录右下角: ({int(x)}, {int(y)})。"
                if status_callback:
                    status_callback(msg)
                else:
                    print(msg)
    ...
    init_msg = f"[{msg_prefix}] 请点击屏幕上的两点：\n1. 点击左上角"
    if status_callback:
